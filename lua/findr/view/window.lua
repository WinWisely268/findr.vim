...
local function tabline_visible()
    ...
    if show_tabs == 2 or (show_tabs == 1 and tabs[2] ~= nil) then return 1 end
    return 0
end
...
function M.new_floating(filetype)
    ...
    if use_border then
        local top = border.top[1] .. string.rep(border.top[2], width - 2) ..
                        border.top[3]
        local mid =
            border.middle[1] .. string.rep(border.middle[2], width - 2) ..
                border.middle[3]
        local bot =
            border.bottom[1] .. string.rep(border.bottom[2], width - 2) ..
                border.bottom[3]
        local border_buf = vim.api.nvim_create_buf(false, true)
        local lines = {top}
        for i = 1, height - 2, 1 do table.insert(lines, mid) end
        ...
        vim.api.nvim_command('au BufWipeout <buffer> exe "silent bw! "' ..
                                 border_buf)
    else
        local buf = vim.api.nvim_create_buf(true, false)
        vim.api.nvim_open_win(buf, true, options)
